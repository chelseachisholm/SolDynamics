...
cat(
  ...
  mu_N[i,t]<- mu_N1[i,t]*z[i,t]
  mu_flo[i,t]<- mu_flo1[i,t]*z[i,t] 
  
  # Models of elevation effects on dem data
  log(mu_N1[i,t]) <- beta_N[1] + beta_N[2]*elev[i] 
  log(mu_flo1[i,t]) <- beta_flo[1] + beta_flo[2]*elev[i] 
  ...
  #Demography
  beta_N[1] ~ dnorm(0, 0.00001)
  beta_N[2] ~ dnorm(0, 0.00001)
  sigma_N ~ dunif(0.00001, 10000)
  tau_N <- 1/(sigma_N*sigma_N)

  beta_flo[1] ~ dnorm(0, 0.00001)
  beta_flo[2] ~ dnorm(0, 0.00001)
  sigma_flo ~ dunif(0.00001, 1000)
  ...
  #Colonization
  log.alpha ~ dunif(0, 200)  #dispersal scale parameter
  alpha <- exp(log.alpha)
...
inits_fn = function() list(z=matrix(data=rep(1, 2741*5), nrow=2741, ncol=5), log.alpha=runif(1,0,10), psi1 = runif(1,0.01,1), taut = 0.001, sigma_flo= runif(1, 0.00001, 10), sigma_N= runif(1, 0.00001, 1000), beta_N=rnorm(2,0,10), beta_flo=rnorm(2,0,10), beta_phi=rnorm(2,0,10), beta_rho=rnorm(2,0,10))

# Specify parameters for which posterior samples are saved
para.names = c('psi1', 'n_occ', 'log.alpha', 'taut', 'sigma_flo', 'sigma_N',
...
parsamples <- run.jags('occ1.txt', data=Data_simple, inits = inits_fn, n.chains = 3, monitor=para.names,
                       adapt = 1000, burnin = 6000, sample = 5000, method='rjparallel', jags.refresh=10)
 #started at 22h, ended at 14h
#new run started at 16h
summary(parsamples)
plot(parsamples)
newparsamples <- extend.jags(parsamples, sample=8000)
...
save(parsamplesv3, file='modeloutput30012021_poisson.RData')
