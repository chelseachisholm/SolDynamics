...
cat(
  ...
  #Colonization
  log.alpha ~ dnorm(0, 0.001)  #dispersal scale parameter
  alpha <- exp(log.alpha)
...
inits_fn = function() list(z=matrix(data=rep(1, 2741*5), nrow=2741, ncol=5), log.alpha=rnorm(1,0,1), psi1 = runif(1,0.01,1), taut = 0.001, sigma_flo= runif(1, 0.00001, 10), sigma_N= runif(1, 0.00001, 1000), beta_N=rnorm(2,0,10), beta_flo=rnorm(2,0,5), beta_phi=rnorm(2,0,5), beta_rho=rnorm(2,0,5))
...
parsamples <- run.jags('occ1.txt', data=Data_simple, inits = inits_fn, n.chains = 3, monitor=para.names,
                       adapt = 1000, burnin = 6000, sample = 10000, method='rjparallel', jags.refresh=10)
 #started at 22h, ended at 14h
#new run started at 16h
summary(parsamples)
plot(parsamples)
newparsamples <- extend.jags(newparsamples, sample=5000)
