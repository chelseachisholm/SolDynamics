...
jags_data$nsite <- 200
jags_data$occ <- jags_data$occ[seq(0,2000,10),]
jags_data$z <- jags_data$z[seq(0,2000,10),]
jags_data$N <- jags_data$N[seq(0,2000,10),]
jags_data$flo <- jags_data$flo[seq(0,2000,10),]
jags_data$elev <- jags_data$elev[seq(0,2000,10)]
jags_data$distmat <- jags_data$distmat[seq(0,2000,10),seq(0,2000,10)]
...
#### SUCCESS 1 (NO ELEVATION EFFECT ON POP) ####
load.module('glm')
...
cat(
  ...
  logit(phi[i,t]) <- beta_phi[1] + beta_phi[2]*elev[i] + beta_phi[3]*totseed[i,t] #+ trand[t] 
  ...
  #Demography
  mu_N ~ dnorm(0, 10)
 # sigma_N ~ dexp(0.5) #0.25 for longer tail, highest probs 0-5
 # tau_N <- pow(sigma_N, -2)  
  tau_N ~ dexp(0.8)  #dgamma(2,1) works well
  

  mu_flo ~ dnorm(0, 5)
 # sigma_flo ~ dgamma(0.75, 0.75) #or dexp(0.5) for highest prob 0-1
 # tau_flo <- pow(sigma_flo, -2)  
 tau_flo ~ dexp(0.8)  #was 10,10 (worked great), 0.1 was ok

# mu N = 5, sd = 1.39 (tau = 0.85)
# mu flo = 3, sd=0.33 (tau =1.85)
...
inits_fn = function() list(z=matrix(data=rep(1, 2741*5), nrow=2741, ncol=5), alpha=rexp(1,1), psi1 = runif(1,0.01,1), tau_flo= rgamma(1, 0.1, 0.1), tau_N= rgamma(1, 0.1, 0.1), mu_N=rnorm(1,0,5), mu_flo=rnorm(1,0,5), beta_phi=rnorm(3,0,1.6), beta_rho=rnorm(2, 0, 1.6)) # mu_N=abs(rnorm(1,0,5)), mu_flo=abs(rnorm(1,0,5)),
...
parsamples <- run.jags('occ1.txt', data=Data_simple, inits = inits_fn, n.chains = 3, monitor=para.names,
                       adapt = 1000, burnin = 50000, thin=30, sample = 10000, method='parallel')
#burn in 75000, samole 10000 to get good SSeff
...
#### SUCCESS 2 (ADDING IN ELEV EFFECT)####
load.module('glm')
...
# Specify a function to generate initial values for the parameters
inits_fn = function() list(z=matrix(data=rep(1, 200*5), nrow=200, ncol=5), alpha=rexp(1,1), psi1 = runif(1,0.01,1), tau_flo= rgamma(1, 0.1, 0.1), tau_N= rgamma(1, 0.1, 0.1), beta_N=rnorm(2, 0, 1.6), beta_flo=rnorm(2, 0, 1.6), beta_phi=rnorm(3,0,1.6), beta_rho=rnorm(2, 0, 1.6)) # mu_N=abs(rnorm(1,0,5)), mu_flo=abs(rnorm(1,0,5)),
...
parsamples <- run.jags('occ1.txt', data=Data_simple, inits = inits_fn, n.chains = 3, monitor=para.names,
                       adapt = 1000, burnin = 50000, thin=50, sample = 20000, method='parallel')
#burn in 75000, sample 10000 to get good SSeff
#started at 1200h, ended at ___
#new run started at 16h
summary(parsamples)
plot(parsamples)
