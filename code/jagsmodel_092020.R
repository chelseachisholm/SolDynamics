...
Data_simple <- list(n.nb = n.nb, NB.mat = NB.mat, D.nb = D.nb, n.sites = nrow(jags_data$occ), t.max = ncol(jags_data$occ), y = jags_data$occ, z=as.matrix(jags_data$z), N=jags_data$N, flo=jags_data$flo, elev=elev) 

# init.z <- as.matrix(jags_data$z)
# 
# init.z[is.na(init.z)] <- 0
# init.z[,6:13] <- init.z[,5]
...
inits_fn = function() list(alpha=0.1, psi1 = 0.1, tau_N= 1, tau_flo= 1, beta_N=runif(2,-3,3), beta_flo=runif(2,-3,3), beta_phi=runif(2,-3,3), p = 0.3, beta_rho=runif(2,-3,3))

jagsModel_sim = jags.model(file= "occ1.txt", data=Data_simple, n.chains = 3, n.adapt= 0, inits = inits_fn)

# Specify parameters for which posterior samples are saved
para.names = c('n_occ') #, 'p_occ', 'p', 'alpha', 'scol', 'sext', 'stur')
               #'beta_rho', 'beta_phi', 'beta_N', 'beta_flo') 

### 4) Continue the MCMC runs with sampling
Samples_sim = coda.samples(jagsModel_sim, variable.names = para.names, n.iter = 1, na.rm=F)
