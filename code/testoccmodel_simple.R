...
cat(
  ...
  logit(Edem[i,t]) <- beta_dem[1] + beta_dem[2]*elev[i]
...
# 3) Specify a function to generate inital values for the parameters
inits_fn = function() list(gamma0=0.1, psi1 = 0.1, mu_dem = 1, tau_dem= 1, mu_flo = 1, tau_flo= 1, mu_pat = 1, tau_pat = 1, beta_dem=runif(2,-3,3), beta_phi=runif(2,-3,3), z = z, p = 0.9)

jagsModel = jags.model(file= "occ1.txt", data=Data_simple, n.chains = 3, n.adapt= 50)

# Specify parameters for which posterior samples are saved
para.names = c('n_occ', 'p', 'gamma0', 'beta_phi[1]', 'beta_phi[2]') 
