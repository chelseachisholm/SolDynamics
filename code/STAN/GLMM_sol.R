...
# Initial values
inits <- function() list(mu = runif(1, 0, 2), alpha = runif(stan_data$nsite, -1, 1), beta = runif(3, -1, 1), sd.alpha = runif(1, 0, 0.1), sd.year = runif(1, 0, 0.1))
...
## Call Stan from R
out <- stan("../GLMM_sol.stan", data = stan_data,
